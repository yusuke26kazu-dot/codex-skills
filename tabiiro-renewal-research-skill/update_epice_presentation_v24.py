...
# U+3007 is IDEOGRAPHIC NUMBER ZERO (〇), used as placeholder in the template
U_CLIENT_NAME_PLACEHOLDER = "\u3007\u3007\u3007\u3007\u3007\u3007\u3007\u3007\u69d8" # 〇〇〇〇〇〇〇〇様 (U+3007 x8 + 様)
...
U_IRRELEVANT_KEYWORDS = [
    "\u90fd\u9053\u5e9c\u7d2c\u5225", # 都道府県別
    "\u25cf\u25cf\u77cc", # ●●県
    "\u25cb\u25cb\u770c", # ○○県 (placeholder for prefecture ranking)
    "\u65c5\u884c\u30d7\u30e9\u30f3\u306e\u4e2d\u3067", # 旅行プランの中で
    "\u65c5\u884c\u30d7\u30e9\u30f3", # 旅行プラン
    "\u65c5\u8272\u30d7\u30e9\u30b9", # 旅色プラス
    "Instagram",
...
# Encoding-safe keyword check: normalize text to NFC and compare
import unicodedata
def text_contains_keyword(text, keyword):
    """Check if keyword is in text, using NFC normalization for robustness."""
    try:
...
def replace_picture_on_slide(slide, img_path, left_cm, top_cm, width_cm, height_cm):
    """Add a picture to slide at exact position, replacing any existing Picture shapes."""
    # Delete existing Picture shapes (type=13) except the logo (small, top-right)
    for shape in list(slide.Shapes):
        try:
            if shape.Type == 13:  # msoPicture
                # Skip the logo (small picture in top-right: ~3.71cm wide)
...
def download_super_theme_slider_images(super_themes_data, img_dir):
    ...
    for i, (name, t_url, rank, slug) in enumerate(super_themes_data):
        ...
        # Fallback to OG image download if not found on theme list page
        if not downloaded:
            print(f"Falling back to OG image download for {slug}")
            download_og_image(t_url, path)
            
        # Crop the downloaded image (slider or OG) to aspect ratio 20.11 x 12.58
...
def process_presentation():
    # Super Theme = 5 items (image path contains s_gourmet/ on LP)
    super_themes_data = [
        ...
        ("\u53e4\u6c11\u5bb6\u30ec\u30b9\u30c8\u30e9\u30f3\u30fb\u30ab\u30d5\u30a7", "https://tabiiro.jp/gourmet/theme/kominka-cafe/ranking/kinki/kyoto/", "4\u4f4d", "kominka-cafe"),
    ...
    # Normal Theme = 2 items (image path is regular theme/ on LP, no gourmet-specific ranking)
    normal_themes_data = [
        ("\u30ef\u30a4\u30f3\u304c\u304a\u3044\u3057\u3044\u30ec\u30b9\u30c8\u30e9\u30f3", "https://tabiiro.jp/theme/wine/", "?", "wine"),
        ("\u5973\u5b50\u4f1a\u306b\u304a\u3059\u3059\u3081\u306e\u304a\u5e97", "https://tabiiro.jp/theme/jyoshikai/", "?", "jyoshikai")
    ]
    
    # Genre ranking slides: only for super themes with 10+ items in the ranking list
    # high-class=20, french=5(skip), kominka=11, pet=4(skip), xmas=20
    genre_rankings_data = [
    ...
    # Download Gourmet Theme list page images for Super Themes (②)
    ...
    for i, (name, url, rank, slug) in enumerate(normal_themes_data):
        ...
        if download_og_image(url, path):
            try:
                img = Image.open(path)
                target_h = int(img.width * (10.77 / 19.15)) # Clop for Theme Feature (③): 19.15 x 10.77
                if target_h < img.height:
                    top = (img.height - target_h) / 2
                    bottom = top + target_h
                    cropped = img.crop((0, top, img.width, bottom))
    ...
    local_temp_path = os.path.abspath("temp_presentation_v24.pptx")
    ...
    # 0. Find references to base slides at the very beginning!
    super_theme_base_idx = find_slide_index_by_text(pres, U_SUPER_THEME, exclude_text=U_SOZAI)
    theme_base_idx = find_slide_index_by_text(pres, U_THEME, exclude_text=U_SUPER_THEME)
    ...
    print(f"Base slides found initially:")
    print(f"  Super Theme: {super_theme_base_idx}")
    print(f"  Theme Feature: {theme_base_idx}")
    print(f"  SEO Article: {seo_base_idx}")
    print(f"  Genre Ranking: {genre_base_idx}")
    ...
    # 2. Process Super Theme slides (②)
    if super_theme_base_slide:
        print(f"Processing Super Theme slides...")
        for i in reversed(range(len(super_themes_data))):
            ...
            # User requested NOT to touch the title text boxes for ② and ③. They remain "スーパーテーマ特集" and "テーマ特集".
            print(f"  Title TextBox left untouched as '{U_SUPER_THEME}'")
            
            img_path = os.path.join(img_dir, f"st_{i}.jpg")
            if os.path.exists(img_path):
                # Super theme (②) image area: left~4.82cm, top~4.35cm, width~20.11cm, height~12.58cm
                replace_picture_on_slide(new_slide, img_path, left_cm=4.82, top_cm=4.35, width_cm=20.11, height_cm=12.58)
                print(f"  Inserted image st_{i}.jpg")
            else:
    ...
    # 3. Process Regular Theme slides (③)
    if theme_base_slide:
        print(f"Processing Theme Feature slides...")
        for i in reversed(range(len(normal_themes_data))):
            ...
            # User requested NOT to touch the title text boxes for ② and ③. They remain "スーパーテーマ特集" and "テーマ特集".
            print(f"  Title TextBox left untouched as '{U_THEME}'")
            
            img_path = os.path.join(img_dir, f"nt_{i}.jpg")
            if os.path.exists(img_path):
                # Theme Feature (③) image area: left~5.28cm, top~5.45cm, width~19.15cm, height~10.77cm
                replace_picture_on_slide(new_slide, img_path, left_cm=5.28, top_cm=5.45, width_cm=19.15, height_cm=10.77)
                print(f"  Inserted image nt_{i}.jpg at Left: 5.28cm, Top: 5.45cm")
            else:
                print(f"  WARNING: image not found: {img_path}")
    ...
    if seo_base_slide:
        ...
        for i, (kw, url, rank, views) in enumerate(reversed(seo_articles)):
            ...
            if os.path.exists(img_path):
                # SEO Article (④) image area: left~4.82cm, top~4.35cm, width~20.11cm, height~11.31cm
                replace_picture_on_slide(new_slide, img_path, left_cm=4.82, top_cm=4.35, width_cm=20.11, height_cm=11.31)
                print(f"  Inserted image seo_{2-i}.jpg")
            else:
                print(f"  WARNING: image not found: {img_path}")
        # Delete original base slide
    ...
    # 5. Process Genre ranking slides
    if genre_base_slide:
        print(f"Processing Genre Ranking slides...")
    ...
    for idx in range(pres.Slides.Count, 0, -1):
        ...
        if text_contains_keyword(text_content, U_SOZAI) or text_contains_keyword(text_content, U_SKIP_INDICATOR_2) or text_contains_keyword(text_content, U_SKIP_INDICATOR):
            slide.Delete()
            print(f"Deleted template skip/material slide {idx} by text check (layout included).")
            continue
            
        # B. Delete ALL other empty or irrelevant slides strictly
        if text_contains_any_keyword(text_content, U_IRRELEVANT_KEYWORDS):
            slide.Delete()
            print(f"Deleted irrelevant slide {idx} containing keyword.")
            continue
            
        # C. Filter magazines: keep only TG5
    ...
    out_path = os.path.abspath(r"G:\マイドライブ\codex-skills\tbiiro-renewal\更新資料\epice_エピス_TG提案書_v24.pptx")
