...
def process_presentation():
    ...
    local_temp_path = os.path.abspath("temp_presentation_v19.pptx")
    ...
    # Template slide 3: TextBox 6 = "テーマ特集", Picture 19 = placeholder image
    # TextBox 10 = "本テーマ特集にて御社をご紹介しています" (do NOT replace this)
    ...
    if theme_base_idx != -1:
        ...
        for i in reversed(range(len(normal_themes_data))):
            ...
            new_slide = base_slide.Duplicate().Item(1)
            # Replace ONLY the title TextBox (short one = テーマ特集 only), not the body text
            for shape in new_slide.Shapes:
                try:
                    if shape.HasTextFrame and shape.TextFrame.HasText:
                        txt = shape.TextFrame.TextRange.Text
                        # Only replace if text is exactly "テーマ特集" (title only)
                        if txt.strip() == U_THEME:
    ...
    out_path = os.path.abspath(r"G:\マイドライブ\codex-skills\tbiiro-renewal\更新資料\epice_エピス_TG提案書_v19.pptx")
