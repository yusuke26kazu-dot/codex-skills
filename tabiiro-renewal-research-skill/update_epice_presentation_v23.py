...
def process_presentation():
    ...
    local_temp_path = os.path.abspath("temp_presentation_v23.pptx")
    ...
    if theme_base_slide:
        ...
        for i in reversed(range(len(normal_themes_data))):
            ...
            if os.path.exists(img_path):
                # Theme Feature (③) image area: left~4.82cm, top~4.35cm, width~19.15cm, height~10.77cm
                replace_picture_on_slide(new_slide, img_path, left_cm=4.82, top_cm=4.35, width_cm=19.15, height_cm=10.77)
                print(f"  Inserted image nt_{i}.jpg")
            else:
                print(f"  WARNING: image not found: {img_path}")
        # Delete original base slide
    ...
    out_path = os.path.abspath(r"G:\マイドライブ\codex-skills\tbiiro-renewal\更新資料\epice_エピス_TG提案書_v23.pptx")
