...
def process_presentation():
    ...
    # Clop to exact aspect ratio of new sizes
    for i, (name, url, rank, slug) in enumerate(super_themes_data):
        ...
        if download_og_image(url, path):
            try:
                img = Image.open(path)
                target_h = int(img.width * (12.58 / 20.11)) # Clop for Super Theme (②): 20.11 x 12.58
    ...
    local_temp_path = os.path.abspath("temp_presentation_v22.pptx")
    ...
    out_path = os.path.abspath(r"G:\マイドライブ\codex-skills\tbiiro-renewal\更新資料\epice_エピス_TG提案書_v22.pptx")
