...
def process_presentation():
    ...
    local_temp_path = os.path.abspath("temp_presentation_v21.pptx")
    ...
    if super_theme_base_slide:
        ...
        for i in reversed(range(len(super_themes_data))):
            ...
            # Replace ONLY the title TextBox (TextBox 9 = "スーパーテーマ特集"), not TextBox 10
            for shape in new_slide.Shapes:
                try:
                    sname = shape.Name
                    if shape.HasTextFrame and shape.TextFrame.HasText:
                        txt = shape.TextFrame.TextRange.Text
                        if U_SUPER_THEME in txt and "\u672c" not in txt[:2]:  # 本 = 'this'
                            shape.TextFrame.TextRange.Replace(U_SUPER_THEME, name)
                            print(f"  Replaced title in {sname}: {repr(txt[:50])}")
                except Exception: pass
            
            img_path = os.path.join(img_dir, f"st_{i}.jpg")
            if os.path.exists(img_path):
                # Super theme image area: left~2.22cm, top~3.79cm, width~19.15cm, height~10.77cm
                replace_picture_on_slide(new_slide, img_path, left_cm=2.22, top_cm=3.79, width_cm=19.15, height_cm=10.77)
    ...
    if theme_base_slide:
        ...
        for i in reversed(range(len(normal_themes_data))):
            ...
            # Replace ONLY the title TextBox using NFC normalization
            import unicodedata
            for shape in new_slide.Shapes:
                try:
                    if shape.HasTextFrame and shape.TextFrame.HasText:
                        txt = shape.TextFrame.TextRange.Text
                        norm_txt = unicodedata.normalize('NFC', txt.strip())
                        norm_theme = unicodedata.normalize('NFC', U_THEME)
                        if norm_txt == norm_theme:
                            shape.TextFrame.TextRange.Text = name
                            print(f"  Set title to: {name}")
                except Exception: pass
            
            img_path = os.path.join(img_dir, f"nt_{i}.jpg")
            if os.path.exists(img_path):
                # Theme image area same as super theme
                replace_picture_on_slide(new_slide, img_path, left_cm=2.22, top_cm=3.79, width_cm=19.15, height_cm=10.77)
    ...
    if seo_base_slide:
        ...
        for i, (kw, url, rank, views) in enumerate(reversed(seo_articles)):
            ...
            if os.path.exists(img_path):
                # SEO image area: left~1.56cm, top~4.67cm, width~20.11cm, height~11.31cm
                replace_picture_on_slide(new_slide, img_path, left_cm=1.56, top_cm=4.67, width_cm=20.11, height_cm=11.31)
    ...
    out_path = os.path.abspath(r"G:\マイドライブ\codex-skills\tbiiro-renewal\更新資料\epice_エピス_TG提案書_v21.pptx")
