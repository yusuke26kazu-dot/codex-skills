    ...
    for shape in shapes:
        try:
            if shape.Type == 6: # msoGroup
                ...
                # Try direct replacement, catching exceptions for non-text shapes safely
                try:
...
def find_slide_index_by_text(pres, search_text, exclude_text=None):
    for i in range(1, pres.Slides.Count + 1):
        ...
        # Safe constraint: Exclude Hidden (non-printable material) slides from base slide selection
...
def process_presentation():
    # 1. Define Gourmet Research parameters precisely
    super_themes_data = [
    ...
    local_temp_path = os.path.abspath("temp_presentation_v15.pptx")
    ...
    # Exclude using U_SOZAI ("素材") to correctly target printable slide U_SUPER_THEME
    ...
    # Exclude using U_SOZAI ("素材") to correctly target printable slide U_THEME
    ...
    for idx in range(pres.Slides.Count, 0, -1):
        ...
        # Ultimate failproof check: Slide transition Hidden property OR U_SOZAI/U_SKIP_INDICATOR text content
        if slide.SlideShowTransition.Hidden:
            slide.Delete()
        ...
        def extract_text(shapes):
            ...
            for shape in shapes:
                ...
                # Direct check catching all text sources
                try:
                    t = shape.TextFrame.TextRange.Text
                    if t: text_content += t + "\n"
                except Exception: pass
        ...
        if U_SOZAI in text_content or U_SKIP_INDICATOR_2 in text_content or U_SKIP_INDICATOR in text_content:
            slide.Delete()
            print(f"Deleted template skip/material slide {idx} by text check.")
            continue
    ...
    out_path = os.path.abspath(r"G:\マイドライブ\codex-skills\tbiiro-renewal\更新資料\epice_エピス_TG提案書_v16.pptx")
