...
async function initHolidays() {
    try {
        ...
        if (response.ok) {
            ...
            console.log('Holidays loaded:', Object.keys(holidays).length);
        } else {
            console.warn('Failed to load holidays API, using fallback.');
            setupFallbackHolidays();
        }
    } catch (error) {
...
// Fallback for Japanese Holidays (2026 calculations/list since the current file is 2026)
function setupFallbackHolidays() {
    // Basic 2026 holiday mapping (May 2026 specifically, since our target is May 2026)
    holidays = {
        "2026-01-01": "元日",
        "2026-01-12": "成人の日",
        "2026-02-11": "建国記念の日",
